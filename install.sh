...
[ -n "$WWW_ROOT_DIR" ] || WWW_ROOT_DIR="/usr/share/webapps/jekyll"
ruby_version="$(command ruby --version 2>/dev/null | sed 's|[(].*||g;s|[a-zAZ]||g;s|^ ||g' | __get_version | grep '^' || true)"
[ -n "$ruby_version" ] && [ "$ruby_version" = "2007" ] || { echo "This requires ruby version 2.7" && exit 1; }
[ "$(command -v gem 2>/dev/null)" ] || { echo "Missing ruby-dev package" && exit 1; }
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
export BUNDLE_HOME=/usr/local/share/bundler
export BUNDLE_APP_CONFIG=/usr/local/share/bundler
export BUNDLE_BIN=/usr/local/bin
export GEM_BIN=/usr/local/bin
export GEM_HOME=/usr/local/share/gem
export JEKYLL_VAR_DIR=/var/jekyll
export JEKYLL_DATA_DIR=/usr/local/share/jekyll
export JEKYLL_BIN=/usr/local/bin
export JEKYLL_ENV=development
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
echo "gem: --no-ri --no-rdoc" >"$HOME/.gemrc"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
mkdir -p "$BUNDLE_HOME" "$JEKYLL_VAR_DIR" "$JEKYLL_DATA_DIR"
chown -R ${USER}:${USER} "$JEKYLL_DATA_DIR" "$JEKYLL_VAR_DIR" "$BUNDLE_HOME"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
yes | gem install jekyll -v 3.9.3 -- --use-system-libraries &&
  yes | gem install html-proofer jekyll-reload jekyll-mentions jekyll-coffeescript jekyll-sass-converter jekyll-commonmark \
    jekyll-paginate jekyll-compose jekyll-assets RedCloth kramdown jemoji s3_website jekyll-sitemap jekyll-redirect-from \
    jekyll-feed jekyll-docs minima jekyll-github-metadata github-pages casjaysdev-jekyll-theme -- --use-system-libraries
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
git -q "https://github.com/casjaysdevdocker/jekyll" "$WWW_ROOT_DIR"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# End application
